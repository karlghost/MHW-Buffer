...
function bindings.get_device()
    if not input_manager then input_manager = sdk.get_managed_singleton("app.GameInputManager") end
...
function bindings.perform(data)
    ...
    if #path == 2 then -- Example: character.sharpness
        if type(on_value) == "boolean" then
            modules[module_index].data[path[2]] = not modules[module_index].data[path[2]]
            utils.send_message(bindings.get_formatted_title(data.path) .. " "..(modules[module_index].data[path[2]] and enabled_text or disabled_text))
        elseif type(on_value) == "number" then
            if modules[module_index].data[path[2]] == -1 then
                modules[module_index].data[path[2]] = on_value
                utils.send_message(bindings.get_formatted_title(data.path) .. " ".. enabled_text)
            else
                modules[module_index].data[path[2]] = -1
    ...
    elseif #path == 3 then -- Example: miscellaneous.ammo_and_coatings.unlimited_ammo
        if type(on_value) == "boolean" then
            modules[module_index].data[path[2]][path[3]] = not modules[module_index].data[path[2]][path[3]]
            utils.send_message(bindings.get_formatted_title(data.path) .. " "..(modules[module_index].data[path[2]][path[3]] and enabled_text or disabled_text))
        elseif type(on_value) == "number" then
            if modules[module_index].data[path[2]][path[3]] == -1 then
                modules[module_index].data[path[2]][path[3]] = on_value
                utils.send_message(bindings.get_formatted_title(data.path) .. " ".. enabled_text)
            else
                modules[module_index].data[path[2]][path[3]] = -1
                utils.send_message(bindings.get_formatted_title(data.path) .. " ".. disabled_text)
            end
        end
    elseif #path == 4 then -- Example: character.conditions_and_blights.blights.fire
        if type(on_value) == "boolean" then
            modules[module_index].data[path[2]][path[3]][path[4]] = not modules[module_index].data[path[2]][path[3]][path[4]]
            utils.send_message(bindings.get_formatted_title(data.path) .. " "..(modules[module_index].data[path[2]][path[3]][path[4]] and enabled_text or disabled_text))
        elseif type(on_value) == "number" then
            if modules[module_index].data[path[2]][path[3]][path[4]] == -1 then
                modules[module_index].data[path[2]][path[3]][path[4]] = on_value
                utils.send_message(bindings.get_formatted_title(data.path) .. " ".. enabled_text)
            else
                modules[module_index].data[path[2]][path[3]][path[4]] = -1
                utils.send_message(bindings.get_formatted_title(data.path) .. " ".. disabled_text)
            end
        end
    end
